...
export function computeTransactionHash(
  ...
  // Convert to JSON string with proxy bigints
  const jsonString = JSON.stringify(transactionWithoutHash, bigIntReplacer);
...
export function getFinancialMetrics(transactions: Transaction[]) {
  ...
  const avgTransactionValue =
    nonDraftTransactions.length > 0
      ? nonDraftTransactions.reduce((sum, tx) => sum + tx.amount, 0) /
        nonDraftTransactions.length
      : 0;
  ...
  const monthlyData = Object.values(transactionsByMonth).sort(
  ...
  // Convert to array and sort by date
  let dailyData = Object.values(transactionsByDay).sort(
    (a, b) => a.date.getTime() - b.date.getTime(),
  );
